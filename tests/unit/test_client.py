...
class TestAutoGeneratedClient(unittest.TestCase):
    ...
    def test_signing_region_does_not_change_client_region(self):
        with mock.patch('botocore.args.RequestSigner') as mock_signer:
            ...
            assert credential_scope_region == call_args[0][1]
    ...
    def test_client_uses_signing_name_from_credential_scope(self):
        with mock.patch('botocore.args.RequestSigner') as mock_signer:
            ...
            service_client = creator.create_client(
                ...
                credentials=self.credentials)
            call_args = mock_signer.call_args
            assert 'MyService' == call_args[0][0]
            assert 'override' == call_args[0][2]
    ...
    def test_client_uses_given_region_name_and_endpoint_url_when_present(self):
        with mock.patch('botocore.args.RequestSigner') as mock_signer:
            ...
            assert service_client.meta.region_name == 'us-west-2'
            call_args = mock_signer.call_args
            assert 'us-west-2' == call_args[0][1]
    ...
    def test_operation_cannot_paginate(self):
        ...
        creator = self.create_client_creator()
        service_client = creator.create_client('myservice', 'us-west-2')
        assert service_client.can_paginate('test_operation') is False
    ...
    def test_service_has_waiter_configs(self):
        ...
        creator = self.create_client_creator()
        service_client = creator.create_client('myservice', 'us-west-2')
        assert sorted(service_client.waiter_names) == sorted(['waiter1', 'waiter2'])
    ...
    def test_client_s3_addressing_style_with_config(self):
        ...
        assert my_client.meta.config.s3['addressing_style'] in 'auto'
    ...
    def test_client_s3_addressing_style_with_config_store(self):
        ...
        creator = self.create_client_creator()
        client = creator.create_client('myservice', 'us-west-2')
        assert client.meta.config.s3['addressing_style'] in 'virtual'
    ...
    def test_client_payload_signing_from_config_store(self):
        ...
        assert my_client.meta.config.s3['payload_signing_enabled']
    ...
    def test_client_s3_accelerate_from_config_store(self):
        ...
        creator = self.create_client_creator()
        my_client = creator.create_client('myservice', 'us-west-2')
        assert my_client.meta.config.s3['use_accelerate_endpoint']
    ...
    def test_error_with_no_wire_code(self):
        ...
        with Stubber(client) as stub:
            ...
            try:
                ...
                assert e.__class__ == exceptions.ClientError
...
class TestConfig(unittest.TestCase):
    ...
    def test_validates_retry_config(self):
        with pytest.raises(
            ...
            match='Cannot provide retry configuration for "not-allowed"'):
...
class TestClientEndpointBridge(unittest.TestCase):
    ...
    def test_guesses_endpoint_as_last_resort(self):
        ...
        assert 'myservice', resolved['signing_name']
        assert 'myservice' == resolved['service_name']
        assert 'v4' == resolved['signature_version']
        assert 'https://myservice.guess.amazonaws.com' == resolved['endpoint_url']
    ...
    def test_uses_us_east_1_by_default_for_s3(self):
        ...
        assert 'us-east-1' == resolved['region_name']
        assert 'us-east-1' == resolved['signing_region']
        assert 'https://s3.amazonaws.com' == resolved['endpoint_url']
    ...
    def test_uses_region_from_client_config_if_available(self):
        ...
        assert 'us-foo-bar' == resolved['region_name']
        assert 'us-foo-bar' == resolved['signing_region']
        assert 'https://test.us-foo-bar.amazonaws.com' == resolved['endpoint_url']
    ...
    def test_can_guess_endpoint_and_use_given_endpoint_url(self):
        ...
        assert 'guess' == resolved['region_name']
        assert 'guess' == resolved['signing_region']
        assert 'http://test.com' == resolved['endpoint_url']
    ...
    def test_can_use_endpoint_url_with_resolved_endpoint(self):
        ...
        assert 'us-west-2' == resolved['region_name']
        assert 'us-west-2' == resolved['signing_region']
        assert 'https://foo' == resolved['endpoint_url']
        assert 'v2' == resolved['signature_version']
    ...
    def test_uses_ssl_common_name_over_hostname_if_present(self):
        ...
        assert 'us-west-2' == resolved['region_name']
        assert 'us-west-2' == resolved['signing_region']
        assert 'https://common-name.com' == resolved['endpoint_url']
    ...
    def test_can_create_http_urls(self):
        ...
        assert 'http://host.com' == resolved['endpoint_url']
    ...
    def test_credential_scope_overrides_signing_region(self):
        ...
        assert 'us-foo-baz' == resolved['region_name']
        assert 'override' == resolved['signing_region']
    ...
    def test_cred_scope_does_not_override_signing_region_if_endpoint_url(self):
        ...
        assert 'us-foo-baz' == resolved['region_name']
        assert 'us-foo-baz' == resolved['signing_region']
        assert 'https://override.com' == resolved['endpoint_url']
    ...
    def test_resolved_region_overrides_region_when_no_endpoint_url(self):
        ...
        assert 'override' == resolved['region_name']
        assert 'override' == resolved['signing_region']
        assert 'https://host.com' == resolved['endpoint_url']
    ...
    def test_uses_signature_version_from_client_config(self):
        ...
        assert 's3' == resolved['signature_version']
    ...
    def test_uses_signature_version_from_client_config_when_guessing(self):
        ...
        bridge = ClientEndpointBridge(resolver, client_config=client_config)
        resolved = bridge.resolve('test', 'us-west-2')
        assert 's3v4' == resolved['signature_version']
    ...
    def test_uses_known_signers_from_list_of_signature_versions(self):
        ...
        bridge = ClientEndpointBridge(resolver)
        resolved = bridge.resolve('test', 'us-west-2')
        assert 'v3https' == resolved['signature_version']
    ...
    def test_uses_service_name_as_signing_name(self):
        ...
        bridge = ClientEndpointBridge(resolver)
        resolved = bridge.resolve('test', 'us-west-2')
        assert 'test' == resolved['signing_name']
    ...
    def test_uses_service_signing_name_when_present_and_no_cred_scope(self):
        ...
        resolved = bridge.resolve('test', 'us-west-2')
        assert 'foo' == resolved['signing_name']
